...
int main()
{
    ...
    char origData[STR_NUM][STR_LEN] = {"Hickory dickory dus. . .", "Are you tired of C++?",
                                       "Well, if you have,", "That's just too bad,",
                                       "There's plenty more for us!"};
    char readData[STR_NUM][STR_LEN] = {{0}};
    ofstream out("./out/Poem.bin", ios::out | ios::binary);
    //   assure(out, "Poem.bin");
    for (int i = 0; i < STR_NUM; i++)
        out.write(origData[i], STR_LEN);
    out.close();

    ifstream in("./out/Poem.bin", ios::in | ios::binary);
    //   assure(in, "Poem.bin");
