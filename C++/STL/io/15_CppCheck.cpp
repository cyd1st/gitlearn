...
void cppCheck(string fileName)
{
    ...
    part[HLINE1] = "//"
                   ": " +
                   part[HEADER];
    part[GUARD1] = "#ifndef _" + part[BASE] + "_H";
    part[GUARD2] = "#define _" + part[BASE] + "_H";
    part[GUARD3] = "#endif // _" + part[BASE] + "_H";
    part[CPPLINE1] = string("//") + ": " + part[IMPLEMENT];
    part[INCLUDE] = "#include \"" + part[HEADER] + "\"";
    // First, try to open existing files:
    ifstream existh(part[HEADER].c_str()), existcpp(part[IMPLEMENT].c_str());
    if (!existh) { // Doesn't exist; create it
        ...
        hfile << existh.rdbuf(); // 将输入文件中的内容，输出到字符流中
        // Check that first three lines conform:
        bool changed = false;
        string s;
        hfile.seekg(0); // 设置读取指针位置
        getline(hfile, s);
        bool lineUsed = false;
    ...
    if (!existcpp) { // Create cpp file
        ...
        newcpp << part[CPPLINE1] << endl << part[INCLUDE] << endl;
    } else { // Already exists; verify it
